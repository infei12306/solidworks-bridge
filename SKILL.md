...
| Slow job, return at once | `run --code "..." --async` 鈫?then `status --id <id>` |
...
`app` (ISldWorks) 路 `doc` (IModelDoc2 or None) 路 `session` 路 `L` (interface
classes) 路 `E` (8199 constants) 路 `cast` 路 `getv` 路 `call` 路 `call_out` 路
`OUTARG` 路 `retry` 路 `OUT` (write your outputs here) 路 `HOME` 路 `log`

[scripts/../../tests/jobs/make_box.py](../tests/jobs/make_box.py) is a worked
example: 50x30x20 mm box 鈫?mass properties 鈫?save 鈫?render.
...
9. **The UI may be Chinese.** 'Front Plane' is `鍓嶈鍩哄噯闈, an extrude is
   `鍑稿彴-鎷変几1`. Never look reference geometry up by name: walk the tree with
...
16. **For a countersink use `IFeatureManager.InsertFeatureChamfer(Options,
    ...
    circular rim IS a countersink - verified: 脴6.5 + 2 x 2.75 = 脴12 at 90 degrees,
...
20. **`SetAddToDB(True)` before creating sketch entities, `False` before the
    ...
    the profile was 48.9 mm虏 wrong. Always log
...
21. **`CreateArc(..., Direction=True)` is counter-clockwise**; a clockwise corner
    passed `True` becomes the **270掳 major arc**. Choose per arc from the shorter
    ...
    not a rounded corner (cost 0.5708 mm虏 here). MODELING.md 搂21.
...
`D:\妗岄潰鏂囦欢\杞︽灦澶嶅埢浜や粯\arduino-mega2560\build_mega.py` and the 93-body 16-channel
relay board at `D:\妗岄潰鏂囦欢\workspace\relay-board\build_relay_board.py` (the latter
...
2. Walk with `swcore.iter_features(doc)`; components come back as features of type
   `Reference`, and the mate folder is the `MateGroup` (its name is `閰嶅悎` on a
   Chinese UI). Take its children with `GetFirstSubFeature`/`GetNextSubFeature`,
   casting each to `IFeature`.
3. Per component: `cast(comp, "IComponent2")` 鈫?`Name2`, `IsFixed`,
   `GetConstrainedStatus`, `GetPathName`, and `GetXform` for the placement.
4. Per mate: `GetSpecificFeature2` 鈫?`cast(..., "IMate2")` 鈫?   `GetMateEntityCount`/`MateEntity(k)` 鈫?`cast(..., "IMateEntity2")` 鈫?   `ReferenceComponent` (cast to `IComponent2` for the name). `IMateEntity2`
...
**Something is stuck**
1. `status --id <id>` 鈫?`RUNNING` with a flat log means the call never returned.
2. `dialogs` 鈫?is there a MODAL window?
